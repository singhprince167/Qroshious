        ...
        <nav className="hidden lg:flex items-center space-x-8" role="navigation">
          ...
          <div className="relative" ref={brandSwitcherRef}>
            <button
              ...
              className="flex items-center space-x-2 px-3 py-2 text-sm font-medium hover:bg-accent rounded-3xl transition-colors"
            >
              Brands
              <ChevronUp
            ...
            {isBrandSwitcherOpen && (
              <div className="absolute right-0 top-full mt-2 w-48 bg-white border rounded-2xl shadow-lg p-2 z-50">
                {brands.map((brand) => (
                  <a
                    key={brand.id}
                    href={brand.link}
          ...
          <div className="px-4 py-6 space-y-4">
            ...
            {navLinks.map((link) => (
              <button
                key={link.href}
                onClick={() => handleNavClick(link.href)}
                className="block w-full text-left py-3 px-4 text-lg font-medium text-foreground hover:bg-muted rounded-lg"
              >
                {link.label}
              </button>
            ))}
            ...
            <div className="pt-4 border-t border-border">
              ...
              {brands.map((brand) => (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block w-full px-4 py-3 hover:bg-muted rounded-lg"
                >
                  {brand.name}
                </a>
              ))}
